...
class FormattedResponse @Inject constructor(private val apiInterface: ApiInterface) {
   ...
   private  fun <T> getResult(result: Response<GenericApiResponse<T>>?, e: Exception? = null): Resource<T> {
    ...
    private  fun <T> getResultNew(result: Response<T>?, e: Exception? = null): Resource<T> {
        if (result != null && result.isSuccessful) {
            appLog.e("sasa-head", result.headers().toString())
            result.body()?.let {
                return Resource.Success(result.body()!!)
            }
            return Resource.Error(result.errorBody().toString(), 1)
              /*  if (it ?: 1 == 0) {
                    it.data?.let { data ->
                        return Resource.Success(data)
                    } ?: run {
                    ...
                    AppLog.e("API Error: Server error -> ${it.message
                        ?: "status is 1"} :: errorCode: 404")
                    return Resource.Error( it.message ?: ErrorMessages(404).getMessage(), result.code())
                }
            } ?: run {
            ...
            e?.let {
                AppLog.e("API Error: Internal error -> ${it.message ?: "Message is null"} :: Code: -1")
//                val errorType: ErrorType = if (e is SocketTimeoutException || e is ConnectException || e is InterruptedIOException) {
...
//                }
//                val message = if (errorType == ErrorType.TimeOutError || errorType == ErrorType.NoInternet) "Please check your internet connection." else it.message
                return Resource.Error( it.message  ?: ErrorMessages(-1).getMessage(), -1)
            } ?: kotlin.run {
                val errorResponse = result?.errorBody()?.string()
                val errorCode =result?.raw()?.code
